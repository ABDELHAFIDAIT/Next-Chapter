...
@section('main')
    ...
    <section class="px-20 flex flex-col gap-5">
        ...
        <div class="flex items-center justify-between py-10">
            <form method="POST" action="{{ route('prisonner.courses.filter') }}" class="flex items-center gap-5">
                @csrf
                <select name="category" class="py-2 px-5 bg-white border border-gray-200 text-sm rounded-md w-64 outline-none">
            ...
            <form method="POST" action="{{ route('prisonner.courses.search') }}" class="flex items-center gap-2 py-2 px-5 bg-white border border-gray-200 text-sm rounded-md outline-none">
                @csrf
                <input required name="search" type="text" class="outline-none w-81" placeholder="Search (Course, Training Program, Keywords, ...)">
