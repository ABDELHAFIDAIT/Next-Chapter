    ...
    <main class="flex">
        ...
        <nav class="flex flex-col justify-between h-screen bg-[#222222] text-white p-4 w-56">
            ...
            <div class="flex flex-col gap-2 text-xs">
                ...
                <a href="{{ route('admin.recruiters') }}" class="flex items-center gap-3 p-3 hover:bg-[#D6FF40] hover:text-black transition-all ease-in-out duration-300 rounded-lg bg-[#D6FF40] text-black">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-[#222] text-gray-200">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">Company Name</th>
                                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">Responsable</th>
                            ...
                            <tbody class="divide-y divide-gray-200 text-sm text-gray-800">
                                @foreach($recruiters as $user)
                                    <tr class="hover:bg-gray-50 transition">
                                        <td class="px-6 py-4">
                                            <img src="{{ asset('storage/'.$user->recruiter->logo) }}" class="w-10 h-10 rounded-md object-cover" alt="photo">
                                        </td>
                                        <td class="px-6 py-4 font-medium">
                                            {{ $user->recruiter->company_name }}
                                        </td>
                                        <td class="px-6 py-4 font-medium">
                    ...
                    @if ($recruiters->hasPages())
                        <nav>
                            <ul class="flex items-center justify-center gap-5">
                                ...
                                @else
                                    <li>
                                        <a href="{{ $recruiters->previousPageUrl() }}" class="">
                                            <svg width="20px" height="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier"> <path d="M14.2893 5.70708C13.8988 5.31655 13.2657 5.31655 12.8751 5.70708L7.98768 10.5993C7.20729 11.3805 7.2076 12.6463 7.98837 13.427L12.8787 18.3174C13.2693 18.7079 13.9024 18.7079 14.293 18.3174C14.6835 17.9269 14.6835 17.2937 14.293 16.9032L10.1073 12.7175C9.71678 12.327 9.71678 11.6939 10.1073 11.3033L14.2893 7.12129C14.6799 6.73077 14.6799 6.0976 14.2893 5.70708Z" fill="#0F0F0F"></path> 
                                ...
                                {{-- Pagination Elements --}}
                                @foreach ($recruiters->getUrlRange(1, $recruiters->lastPage()) as $page => $url)
                                    @if ($page == $recruiters->currentPage())
                                ...
                                {{-- Next Page Link --}}
                                @if ($recruiters->hasMorePages())
                                    <li>
                                        <a href="{{ $recruiters->nextPageUrl() }}" class="">
                                            <svg width="20px" height="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <section id="user-infos-popup" class="hidden flex items-center justify-center fixed inset-0 bg-[rgba(0,0,0,0.7)] z-50">
            <div class="bg-[#EDF7F1] flex flex-col gap-5 p-5 rounded-md w-5/6 text-black">
                ...
                <div class="flex flex-col gap-5 max-h-[70vh] overflow-auto">
                    <div class="flex items-center gap-10 pb-5 border-b border-b-gray-400">
                        <div class="flex items-center gap-10 px-10">
                            <div>
                                {{-- photo --}}
                                <img id="photo" src="" class="h-36 w-36 rounded-md">
                            </div>
                            <div class="flex flex-col gap-10">
                                {{-- full name --}}
                                <h1 id="company_name" class="text-3xl font-semibold"></h1>
                                <div class="flex items-center gap-5">
                    ...
                    <div class="grid grid-cols-3 gap-10 pb-5 px-10 border-b border-b-gray-400">
                        <h1 class="font-medium text-lg">Responsable </h1>
                        <p class="col-span-2 font-light text-sm text-justify" id="responsable"></p>
                    </div>
                    <div class="grid grid-cols-3 gap-10 pb-5 px-10 border-b border-b-gray-400">
                        <h1 class="font-medium text-lg">Sector </h1>
                        <p class="col-span-2 font-light text-sm text-justify" id="sector"></p>
                    </div>
                    <div class="grid grid-cols-3 gap-10 pb-5 px-10 border-b border-b-gray-400">
    ...
    <script>
        ...
        openUserInfosPopupButtons.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                console.log(user);
                document.querySelector('#photo').src = `{{ asset('storage') }}/${user.recruiter.logo}`;
                document.querySelector('#company_name').innerText = `${user.recruiter.company_name}`;
                document.querySelector('#responsable').innerText = `${user.f_name} ${user.l_name}`;
                document.querySelector('#email').href = `mailto:${user.email}`;
                document.querySelector('#email').innerText = user.email;
                document.querySelector('#sector').innerText = user.recruiter.sector;
                document.querySelector('#phone').innerText = user.phone;
                document.querySelector('#city').innerText = user.recruiter.city.name;
                document.querySelector('#about').innerText = user.recruiter.about;
